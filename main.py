...
def _generate_wyckoff_structures(trainer, args):
    """Call trainer.generate_structures and apply optional subsampling."""
    ...
    print(f"Generation in total took {time.time() - generation_start_time} seconds")
...
def generate_tensors(trainer, args):
    generation_start_time = time.time()
...
def generate(args):
    ...
    if args.generate_mode == GenerationMode.WyckoffTensors:
        generate_tensors(trainer, args)
        return
    
    generated_wp = _generate_wyckoff_structures(trainer, args)
